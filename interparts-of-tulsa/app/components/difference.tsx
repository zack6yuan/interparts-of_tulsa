...
export default function TheDifference() {
  ...
  const serviceList = [
    ...
    {
      icon: <Leaf size={28} />, // Optional swap: <Activity size={28} /> or <CheckCircle size={28} /> work great here too
      ...
      <div className="w-full max-w-7xl mx-auto px-6 md:px-14 relative z-10 flex flex-col gap-12">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-white/10 pb-8">
          <div className="flex flex-col items-start font-bebas tracking-wide leading-[0.95]">
            <p className="text-3xl mb-4 text-white/30">// Why Choose Us</p>
            <h1 className="text-6xl md:text-8xl text-white">The Interparts</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
          {serviceList.map((service, index) => (
            <div
              key={index}
              className="bg-white/3 border border-white/10 hover:border-gold/40 p-6 rounded-sm transition-all duration-300 group hover:bg-white/5"
            >
